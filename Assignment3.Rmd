...
gathered web based data from the World Bank API, as well as from the Emissions Database for Global Atmospheric Research (EDGAR) from the European Comission website. We merged the data sets, and conducted basic descriptive and inferential statistics to address the effect of inequality on environment. With this we aim to 

```{r databases, warning=FALSE, error=FALSE, message=FALSE, echo=FALSE}
...
##### We access the World Development Indicators from the World Bank API direcly from R and we download the GINI coefficient indicator and also GDP per capita in case we need it as a control variable. We use the data from 2010 since it is the last one with the GINI information for the highest CO2 emitter in the world, the United States.
...
##### We check the CO2 dataframe that we just downloaded and see that we don't need the first 13 rows so we subset them. Also, we only need the last column with the info of the CO2 emissions of 2013. We label that column since it does not have a name from the import process.
